...
import { Bot, Users, Calendar, CheckCircle2, Lock, ArrowRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { usePlan } from "@/contexts/AppContext";
import { Button } from "@/components/ui/button";

type SecretaryStatus = "online" | "operating" | "offline";
...
const mockSecretaryData = {
  status: "online" as SecretaryStatus,
  clientsBeingServed: 2,
  clientsServedToday: 18,
  appointmentsToday: 12,
};

export const SecretaryStatusCard = () => {
  const { isPro } = usePlan();
  const { status, clientsBeingServed, clientsServedToday, appointmentsToday } = mockSecretaryData;
        ...
        <CardContent className="space-y-4 pt-0">
          ...
          <div className="p-4 rounded-xl bg-orange-100/80 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800/50">
            ...
            <p className="text-xs text-orange-700/80 dark:text-orange-300/70">
              Enquanto você dorme, a Secretária Online poderia estar agendando clientes automaticamente 24 horas por dia.
            </p>
          </div>
      ...
      <CardContent className="space-y-3 pt-2">
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-secondary/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">Atendendo agora</span>
          </div>
          <span className="text-sm font-bold text-foreground">{clientsBeingServed}</span>
        </div>
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-secondary/50">
          ...
          <span className="text-sm font-bold text-foreground">{clientsServedToday}</span>
        </div>
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-secondary/50">
          ...
          <span className="text-sm font-bold text-foreground">{appointmentsToday}</span>
        </div>
      </CardContent>
    </Card>
  );
